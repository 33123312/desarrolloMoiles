...
public class MainActivity extends AppCompatActivity {
    ...
    private void sacadaDeCarta(){
        ...
        oponentCards[contCards].setImageResource(newCard.draw);
        if(newCard.draw  + cuentaIa > 21)
            newCard.value = 1;
        ...
        oponentCards[contCards].setImageResource(newCard.draw);
        if(newCard.draw  + cuentaIa > 21)
            askForAzValue(newCard);
        cuentaIa+= newCard.value;

        evaluateWinConditions();
    }

    private void askForAzValue(Card gottenCard){

        new AlertDialog.Builder(this)
                .setTitle("Elige el vlor del Az")
                .setMessage("El Az puede valer 1 o 11, elige con prudencia")
                .setPositiveButton("1",
    ...
    private void evaluateWinConditions(){
        ...
        else if  (cuentaIa > 21  &&	cuentaJugador > 21){
            if(cuentaJugador > cuentaIa)
                setWinner("El ganador es el Oponente!");
            else
                setWinner("El ganador es el Jugador!");
